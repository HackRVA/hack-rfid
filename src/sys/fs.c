...
void fs_unmount(void) { lfs_unmount(&lfs); }
...
static size_t fs_read_acl(void *ptr, size_t size) {
  ...
  if (err < 0) {
    printf("[fs_read_acl] 'acl' file not found. Creating a new file.\n");

    err = lfs_file_open(&lfs, &f, "acl",
                        LFS_O_WRONLY | LFS_O_CREAT | LFS_O_TRUNC);
    if (err < 0) {
      printf("[fs_read_acl] Failed to create 'acl' file, err=%d\n", err);
      return 0;
    }
...
size_t fs_write_data(File file, const void *ptr, size_t size) {
  lfs_file_t f;
  lfs_file_open(&lfs, &f, "acl", LFS_O_WRONLY | LFS_O_CREAT | LFS_O_TRUNC);
  lfs_ssize_t bw = lfs_file_write(&lfs, (lfs_file_t *)file.handle, ptr, size);
  lfs_file_close(&lfs, &f);
...
void fs_print_contents(void) {
  ...
  while (true) {
    ...
    if (res < 0) {
      printf("[fs_print_contents] Error reading dir entry: %d\n", res);
      break;
    }
    if (res == 0) {
      break;
