...
static inline void cs_select(MFRC522_t *dev)
{
	gpio_put(dev->cs_pin, 0);
}

static inline void cs_deselect(MFRC522_t *dev)
{
 ...
 */
static void MFRC522_calculate_crc(
	MFRC522_t *dev, const uint8_t *data, size_t length, uint8_t *result)
{
...
static uint8_t MFRC522_to_card(MFRC522_t *dev, uint8_t cmd,
	const uint8_t *sendData, size_t sendLen, uint8_t *backData,
	size_t *backLen, uint8_t *validBits)
{
	...
	MFRC522_write_register(
		dev, CommIEnReg, irqEn | 0x80);	   // enable interrupts
	...
	if (!(errorVal & 0x1B)) {
		...
		if (n & irqEn & 0x01) {
			...
			/*printf("[WARN] No tag error (interrupt says no
			 * tag?).\n");*/
		}
		if (cmd == PCD_TRANSCEIVE) {
			// Number of bytes in FIFO
			uint8_t fifoLevel =
				MFRC522_read_register(dev, FIFOLevelReg);
			lastBits =
				MFRC522_read_register(dev, ControlReg) & 0x07;
			if (validBits) {
				if (lastBits) {
					*validBits =
						(fifoLevel - 1) * 8 + lastBits;
				} else {
			...
			if (fifoLevel > *backLen) {
				// If there's more data than the buffer can
				// hold, limit it
				fifoLevel = *backLen;
			}
			// Read the FIFO
			for (i = 0; i < fifoLevel; i++) {
				backData[i] =
					MFRC522_read_register(dev, FIFODataReg);
			}
			*backLen = fifoLevel;
		}
	} else {
		...
		status = MFRC522_ERR;
		printf("[ERR] MFRC522_to_card() => ErrorReg reported "
		       "errors:\n");
...
void MFRC522_init(MFRC522_t *dev, spi_inst_t *spi_port, uint cs_pin,
	uint rst_pin, uint sck_pin, uint mosi_pin, uint miso_pin, uint baudrate)
{
...
uint8_t MFRC522_request(MFRC522_t *dev, uint8_t mode, uint8_t *outBits)
{
	...
	uint8_t status = MFRC522_to_card(dev, PCD_TRANSCEIVE, cmdBuffer, 1,
...
uint8_t MFRC522_anticoll(
	MFRC522_t *dev, uint8_t antiCollVal, uint8_t *serialOut)
{
	uint8_t serChk = 0;
	...
	uint8_t status = MFRC522_to_card(dev, PCD_TRANSCEIVE, cmdBuffer, 2,
		backData, &backLen, &validBits);
	...
	if (status == MFRC522_OK) {
		if (backLen == 5) {
			...
			if (serChk != backData[4]) {
				printf("[ERR] anticoll: XOR check failed. "
				       "(serChk=0x%02X, BCC=0x%02X)\n",
					serChk, backData[4]);
				status = MFRC522_ERR;
			} else {
 ...
 */
uint8_t MFRC522_select_tag(MFRC522_t *dev, const uint8_t *uid, uint8_t uid_size)
{
	...
	cmdBuffer[0] = PICC_ANTICOLL1; // Typically 0x93 if we are selecting
				       // from anticoll1
	cmdBuffer[1] = 0x70;
	...
	uint8_t status = MFRC522_to_card(dev, PCD_TRANSCEIVE, cmdBuffer,
		dataLen, backData, &backLen, &validBits);
	if ((status == MFRC522_OK) && (backLen == 3) && (validBits == 0x18)) {
		return MFRC522_OK;
	}
	printf("[ERR] select_tag: Could not select UID. status=%d, "
	       "backLen=%zu, "
	       "validBits=0x%02X\n",
	...
	const uint8_t *sectorKey, const uint8_t *uid)
{
	...
	uint8_t status = MFRC522_to_card(dev, PCD_AUTHENT, packet,
		sizeof(packet), backData, &backLen, &validBits);
...
uint8_t MFRC522_read(MFRC522_t *dev, uint8_t blockAddr, uint8_t *recvData)
{
	...
	uint8_t status = MFRC522_to_card(dev, PCD_TRANSCEIVE, cmdBuffer, 4,
		backData, &backLen, &validBits);
 ...
 */
uint8_t MFRC522_write(
	MFRC522_t *dev, uint8_t blockAddr, const uint8_t *writeData)
{
	...
	status = MFRC522_to_card(
		dev, PCD_TRANSCEIVE, buf, 18, backData, &backLen, &validBits);
	if ((status != MFRC522_OK) || (backLen != 1) ||
		((backData[0] & 0x0F) != 0x0A)) {
		return MFRC522_ERR;
	}
...
uint8_t MFRC522_read_sector_block(MFRC522_t *dev, const uint8_t *uid,
	uint8_t sector, uint8_t block, const uint8_t *keyA, const uint8_t *keyB,
	uint8_t *outData)
{
	...
	if (keyA != NULL) {
		// Auth with key A
		status = MFRC522_auth(
			dev, PICC_AUTHENT1A, absoluteBlock, keyA, uid);
	} else if (keyB != NULL) {
		// Auth with key B
		status = MFRC522_auth(
			dev, PICC_AUTHENT1B, absoluteBlock, keyB, uid);
	}
	if (status != MFRC522_OK) {
		return MFRC522_ERR;
	}
...
uint8_t MFRC522_write_sector_block(MFRC522_t *dev, const uint8_t *uid,
	uint8_t sector, uint8_t block, const uint8_t *data16,
